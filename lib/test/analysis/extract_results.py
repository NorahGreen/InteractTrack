...
def _normalize_scene_vector(raw_vec, dim=6):
    """
    Convert arbitrary raw scene annotations to a fixed-length 0/1 list.
    Args:
        raw_vec: Iterable of numbers or None.
        dim:     Desired length (defaults to 6 scene types).
    """
    ...
    arr = np.asarray(raw_vec).reshape(-1).tolist()
    norm = []
    for v in arr[:dim]:
        try:
            norm.append(1 if float(v) >= 1 else 0)
        except Exception:
            norm.append(0)
...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    # After length alignment, it is safe to refer to anno_bb[i] inside the zero-width fixup.
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq in dataset:
        ...
        raw_vec = getattr(seq, 'scene_vec', None)
        if scene_dim is None and raw_vec is not None:
            try:
                scene_dim = int(np.asarray(raw_vec).reshape(-1).shape[0])
            except Exception:
                scene_dim = None
        scene_vectors.append(_normalize_scene_vector(raw_vec, dim=scene_dim or 6))
